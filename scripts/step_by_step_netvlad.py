...
def default_config():
    return {
        ...
        "features": {
            ...
            # Regional descriptor: 이미지를 grid로 분할해 공간 정보 보존
            # [rows, cols] 예: [1,3] = 좌/중/우 수평 3분할
            # 최종 dim = base_dim × (rows×cols + 1)  (+1 = 전체 이미지)
            "regional_grid": [1, 3],
...
# =============================================================================
# STEP 3: Global Descriptors  (EigenPlaces > NetVLAD > HSV histogram)
# =============================================================================
def step3_global_desc(rendered, config, output_dir):
    """
    렌더링된 이미지 전체에 대해 global descriptor 추출.
    RGB → grayscale 변환 후 NetVLAD regional descriptor.
    """
    print("\n" + "="*60 + "\nSTEP 3: Global descriptors\n" + "="*60)
    import torch
    fc  = config["features"]
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    base_dim    = fc.get("global_desc_dim", 512)
    grid_rows, grid_cols = fc.get("regional_grid", [1, 3])
    n_regions   = grid_rows * grid_cols + 1   # +1 = 전체 이미지
    gd          = base_dim * n_regions
    print(f"  Regional grid: {grid_rows}×{grid_cols}  "
          f"(+1 full) = {n_regions} regions  →  dim={gd}")

    # ── NetVLAD 로드 ──────────────────────────────────────────────────
    gm = torch.jit.load(fc.get("global_model", "models/netvlad.pt"), map_location=dev)
    gm.eval()
    print(f"  Global model: NetVLAD ({fc.get('global_model', 'models/netvlad.pt')})")

    def _extract_base(rgb_img):
        # RGB → grayscale 3ch: 색상 domain gap 제거, 구조/texture에 집중
        gray = cv2.cvtColor(rgb_img, cv2.COLOR_RGB2GRAY)
        gray3 = np.stack([gray, gray, gray], axis=2)
        t = cv2.resize(gray3, (224, 224)).astype(np.float32) / 255.0
        t = torch.from_numpy(t.transpose(2, 0, 1)).unsqueeze(0).to(dev)
        with torch.no_grad():
            d = gm(t).cpu().numpy().flatten()
        if len(d) < base_dim: d = np.pad(d, (0, base_dim - len(d)))
        return d[:base_dim].astype(np.float32)

    def extract_descriptor(rgb_img):
        return build_regional_descriptor(rgb_img, _extract_base,
                                         grid_rows, grid_cols)

    for i, r in enumerate(rendered):
        rgb = r["rgb"] if isinstance(r.get("rgb"), np.ndarray) \
              else cv2.cvtColor(cv2.imread(r["rgb_path"]), cv2.COLOR_BGR2RGB)
        rgb_desc = extract_descriptor(rgb)
    ...
    ax[0].set_title(f"Similarity {nv}x{nv}")
    ...
    fig.suptitle(f"Step 3: Global descriptors (NetVLAD grayscale)", fontsize=14)
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, "step3_global_desc.png"), dpi=150); plt.close()
    print(f"  Saved: step3_global_desc.png")

    pickle.dump(_slim(rendered), open(os.path.join(output_dir, "step3_data.pkl"), "wb"))
    return rendered
...
def step4_build_db(rendered, output_dir):
    ...
    db = {"global_descs": global_descs_normed, "kdtree": kdtree, "entries": entries}
...
def step5_retrieval(query_image_path, db, config, output_dir):
    ...
    base_dim    = fc.get("global_desc_dim", 512)
    grid_rows, grid_cols = fc.get("regional_grid", [1, 3])
    n_regions   = grid_rows * grid_cols + 1
    gd_dim      = base_dim * n_regions

    # 모델을 함수 캐시에 저장 (같은 프로세스 내 재호출 시 재사용)
    _cache = step5_retrieval.__dict__
    if "_model" not in _cache:
        gm = torch.jit.load(fc.get("global_model", "models/netvlad.pt"), map_location=dev)
        gm.eval()
        _cache["_model"] = gm
        print(f"  Global model: NetVLAD (will reuse)")

    model  = _cache["_model"]
    gd_dim = base_dim * n_regions
    print(f"  Regional grid: {grid_rows}×{grid_cols} (+1 full) = {n_regions} regions  dim={gd_dim}")

    def _base_descriptor(rgb_img):
        # RGB → grayscale 3ch: 색상 domain gap 제거, 구조/texture에 집중
        gray = cv2.cvtColor(rgb_img, cv2.COLOR_RGB2GRAY)
        gray3 = np.stack([gray, gray, gray], axis=2)
        t = cv2.resize(gray3, (224, 224)).astype(np.float32) / 255.0
        t = torch.from_numpy(t.transpose(2, 0, 1)).unsqueeze(0).to(dev)
        with torch.no_grad():
            d = model(t).cpu().numpy().flatten()
        if len(d) < base_dim: d = np.pad(d, (0, base_dim - len(d)))
        return d[:base_dim].astype(np.float32)

    q_rgb_desc = build_regional_descriptor(query_rgb, _base_descriptor,
                                           grid_rows, grid_cols)
    q_gd_norm = q_rgb_desc
...
def step6_match(step5_data, config, output_dir):
    ...
    ax.imshow(canvas)
    if len(best_mkpts_q) > 0:
        cmap_v = plt.cm.RdYlGn(best_confs / (best_confs.max() + 1e-8))
        step_v = max(1, len(best_mkpts_q) // 200)
        for i in range(0, len(best_mkpts_q), step_v):
            ax.plot([best_mkpts_q[i,0], best_mkpts_r[i,0]+w1],
                    [best_mkpts_q[i,1], best_mkpts_r[i,1]],
                    c=cmap_v[i], alpha=0.5, linewidth=0.8)
        ax.scatter(best_mkpts_q[:,0], best_mkpts_q[:,1], c="cyan",   s=8, zorder=3)
        ax.scatter(best_mkpts_r[:,0]+w1, best_mkpts_r[:,1], c="yellow", s=8, zorder=3)
...
def main():
    ...
    if run_offline or args.step == "3_global_desc":
        rendered = step3_global_desc(rendered, config, args.output_dir)
    elif need_gd:
        rendered = _load(args.output_dir, "step3_data.pkl") or rendered
